...
به‌جای آن، بعد از نوشتن فایل، یک خلاصهٔ کوتاه فارسی در ترمینال نمایش بده (بخش 6).
...
- `current` — مقدار `repo_commit` نقشه با `HEAD` یکی است
...
اگر فایل `.audit/project-map.md` وجود نداشت، بازرسی شروع نمی‌شود:
...
قبل از نوشتن فایل، این موارد را چک کن.

**گام صفر:** فایل `project-map.md` وجود دارد. اگر نه، اصلاً فایلی ننویس (بخش 3.1).
